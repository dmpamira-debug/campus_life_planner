...
function toCanonicalMinutes(value, unit) {
	return unit === 'hours' ? hoursToMinutes(value) : roundClean(value);
}

function fromCanonicalMinutes(minutes, unit) {
...
function formatDuration(minutesValue, unit) {
...
function getValidationErrors(rec) {
...
const loadTarget = () => Number(localStorage.getItem(TARGET_KEY) || 0);
const saveTarget = value => localStorage.setItem(TARGET_KEY, value);

const UNIT_KEY = 'campusPlannerUnit';
const loadUnit = () => localStorage.getItem(UNIT_KEY) || 'minutes';
const saveUnit = value => localStorage.setItem(UNIT_KEY, value);
